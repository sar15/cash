import { demoData, demoValueRules, forecastMonths } from './src/lib/demo-data';
...
const result = runForecastEngine(defaultEngineOptions);
...
for (let i = 0; i < months.length; i++) {
  ...
  const openingCash = i === 0 
    ? (result.rawIntegrationResults[0].bs.cash - result.rawIntegrationResults[0].cf.netCashFlow) 
    : result.rawIntegrationResults[i-1].bs.cash;
  const cfDiff = Math.abs(
    m.bs.cash - (openingCash + m.cf.operatingCashFlow + m.cf.investingCashFlow + m.cf.financingCashFlow)
  );
...
const result2 = runForecastEngine({
  ...defaultEngineOptions,
  valueRules: directEntryRules,
});
...
// Verify three-way still balances after direct entry change
